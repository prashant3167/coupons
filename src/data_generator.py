...
def create_coupons(count):
    ...
    for i in range(count):
        ...
        coupon = {
            "code" : coupon_id_generator(random.randint(5,8),random.choice(coupons_prefix)),
            "rule_id" : coupon_rules[select_rule]['rule_id'],
            "created_on" : fake.date_time_between_dates(coupon_rules[select_rule]["created_on"],coupon_rules[select_rule]["created_on"]+timedelta(days=20)),
            "created_by": random.choice(coupon_creators),
            "used_count" : random.randint(0,coupon_rules[select_rule]["can_be_used"]+1)
        }
        coupon["expiration_date"]=fake.date_time_between_dates(coupon["created_on"]+timedelta(days=1),coupon["created_on"]+timedelta(days=20))
...
def create_coupon_rules(count):
    ...
    for i in range(count):
        ...
        coupon_rule = {
            ...
            f"{'absolute' if bool(random.getrandbits(1)) else 'percentage'}": random.randint(1,80),
